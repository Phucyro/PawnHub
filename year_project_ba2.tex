...
\textit{GhostPawn} est utilisé dans le cas où un pion peut effectuer une prise en passant.En effet, lorsqu'un pion avance de 2 cases, il laisse sur la case derrière lui, une pièce non visible pour le joueur.Si le pion adverse se trouve juste à coté de cette pièce,la prise en passant pourra être effectuer car le jeu considère que le \textit{move} du pion adverse est valide car il prend le \textit{GhostPawn}.
\begin{enumerate}
\item
...
\paragraph{Mode Alice} Les méthodes et attributs des pièces sont les mêmes à l'exception d'un attribut 2Dimension pour savoir dans quel dimension se trouve la pièce (et donc une méthode swapDimension). A noter que AlicePawn hérite de BasicPawn et non de Pawn puisqu'il n'y a pas de prises en passant dans le mode Alice.
