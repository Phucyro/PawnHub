...
\textbf{Move :}est une méthode qui selon \textit{checkMove} effectue ou  non, le mouvement demandé par le joueur sur la pièce.Cette méthode effectue d'abord un \texit{CheckMove} à l'endroit du coup désiré par le joueur. Si, le coup est légal, elle effectue un \textit{doMove} ce qui permet d'avancer la pièce à l'endroit du coup.Dans le cas où le coup du \textit{King} le rend en échec, on effectue un \textit{ReverseMove}. \\
\end{enumerate}
