...
bool Knight::_checkMove(Coordinate end, Board* board, Game& game, bool careOfMoving){
	...
	&& ((!_isPlaceFree(end, board, careOfMoving)) ? board->getCase(end)->getColor() != this->getColor() : true); //true if the color of the piece taken is not the same
}
