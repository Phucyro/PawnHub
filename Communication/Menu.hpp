...
void authentificationMenu(MenuHandler* menu, Client* client){
  ...
  while (!client->isIdentified()){
    ...
    menu->refresh_board();
    menu->init_choicesw();
    int choice = menu->get_choice({"Login", "Sign Up"});
    menu->clear_windows();
    ...
    if (choice == 1){
      menu->init_dataw();
      confirmation = menu->get_infos("Password :");
    }
...
void removeFriendMenu(MenuHandler* menu, Client* client){
  ...
  while (command != "/quit"){
    ...
    if (!checkFriendInputFormat(menu, split)){
      ...
      menu->print_warning("You are not friend with this person");
      menu->refresh_board();
      continue;
    }
...
void cancelRequestMenu(MenuHandler* menu, Client* client){
  ...
  while (command != "/quit"){
    ...
    if (!checkFriendInputFormat(menu, split)){
      ...
      menu->print_warning("You didn't send a request to this person");
      menu->refresh_board();
      continue;
    }
...
void chatMenu(MenuHandler* menu, Client* client){
  menu->clear_windows();
  menu->print_warning2("Quit : /quit   Change channel : /change [name]");
  menu->refresh_board();
  ...
  while (command != "/quit"){
    ...
    if (split[0] == "/change"){
      if (split.size() != 2){
        ...
        menu->print_warning("You can not chat with youself");
        menu->refresh_board();
      }
...
void mainMenu(MenuHandler* menu, Client* client){
  ...
  while (!leave){
    menu->clear_windows();
    menu->init_choicesw();
    int choice = menu->get_choice({"Play", "Friends", "Chat", "Statistics", "Quit"});
    menu->refresh_board();
